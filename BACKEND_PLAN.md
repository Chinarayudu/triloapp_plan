...
- **Auto-end on disconnect — built (bug fix)**: the exact same class of gap as presence's auto-offline fix below — `live_broadcasts.status` used to be cleared *only* by an explicit `POST /live/broadcasts/:id/end`, so a host whose connection dropped mid-broadcast (crash, closed tab, network loss, a stale deployed frontend bundle failing to load) left their own row stuck `"live"` forever, permanently 409-ing every future `POST /live/broadcasts` ("You already have a live broadcast running") with no recovery except an admin's force-end. Fixed in `realtime/socket.ts`'s `checkAbandonedBroadcast`: on disconnect, after a grace period (`env.LIVE_BROADCAST_DISCONNECT_GRACE_MS`, default 15s — long enough to survive a quick reconnect/page-reload, unlike presence's instant correction, since ending a broadcast is far more disruptive to active viewers than a status-dot flicker) with no reconnect, the broadcast is auto-ended and `live:ended` fans out to the room the same way a real end does.
...
1. **User-facing currency** — the Figma shows the wallet/recharge/call-spend entirely in "beans," but BRD.md is explicit beans are the host's internal earnings unit only. **Resolved (revised 2026-09-24): the User app shows real currency (₹) only — no beans anywhere on the user side.** Every user-facing amount is integer paise (`GET /wallet` → `{ balancePaise }`, recharge packages → `pricePaise`/`mrpPaise`), rendered as paise / 100. The earlier "beans as a display-only label" approach (a flat `paiseToDisplayBeans` constant plus a separate per-package `displayBeans` marketing number) was removed: the two numbers never agreed, so users saw fewer beans than a package promised (see BUG_HISTORY.md, 2026-09-24).
...
### One phone number, one account per role — built (follow-up)

Splitting the API by app surfaced (and motivated fixing) a leftover assumption from before the split existed: `users.phone` was unique *globally*, so the same phone number could only ever back one account, regardless of role. Verifying OTP for an already-registered phone number just logged into whatever account already existed there — a phone that signed up as a User first could never separately become a Host; the Host app's own OTP verify would silently hand back the existing User account, role and all. Since User and Host are now genuinely separate apps hitting separate API surfaces, that coupling no longer made sense — a real user of both apps expects two independent identities, not one bleeding into the other.

- **Fix**: `db/schema.ts`'s `users` table is now unique per `(phone, role)`, not `phone` alone (migration `0017_hard_unus.sql`). `findUserByPhone` (`users.service.ts`) now takes a required `role` — there's no meaningful "the account for this phone" anymore, only "the account for this phone *and this role*". `auth.routes.ts`'s otp/verify looks up strictly by `(phone, role)`, so `/user/auth/otp/verify` can never see, return, or escalate into a phone's separate `/host/auth/...` account, and vice versa — they're different rows with different ids/wallets from the moment either is created. `createSubAdmin`'s and `db:seed-admin`'s phone-uniqueness checks were scoped the same way (to `sub_admin`/`admin` respectively).
