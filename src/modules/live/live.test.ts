...
describe("Live broadcasting: lifecycle", () => {
  it("starts a broadcast and lists it with a live viewer count", async () => {
    ...
    const list = await request(app).get("/host/live/broadcasts").set("Authorization", `Bearer ${host.accessToken}`);
  ...
  it("tracks concurrent viewers and peak count as users join and leave", async () => {
    ...
    await request(app).post(`/user/live/broadcasts/${broadcastId}/join`).set("Authorization", `Bearer ${user1.accessToken}`);

    await request(app).post(`/user/live/broadcasts/${broadcastId}/join`).set("Authorization", `Bearer ${user2.accessToken}`);

    let single = await request(app).get(`/host/live/broadcasts/${broadcastId}`).set("Authorization", `Bearer ${host.accessToken}`);
    expect(single.body.peakViewerCount).toBe(2);

    await request(app).post(`/user/live/broadcasts/${broadcastId}/leave`).set("Authorization", `Bearer ${user2.accessToken}`);
    ...
    single = await request(app).get(`/host/live/broadcasts/${broadcastId}`).set("Authorization", `Bearer ${host.accessToken}`);
    expect(single.body.peakViewerCount).toBe(2);
  });
  ...
  it("rejects joining a broadcast that doesn't exist", async () => {
    ...
    const res = await request(app)
      .post("/user/live/broadcasts/00000000-0000-0000-0000-000000000000/join")
  ...
  it("ends a broadcast — status becomes ended and it drops off the live list", async () => {
    ...
    const end = await request(app)
      .post(`/host/live/broadcasts/${broadcastId}/end`)
  ...
  it("rejects a non-host ending someone else's broadcast", async () => {
    ...
    const res = await request(app)
      .post(`/host/live/broadcasts/${start.body.broadcastId}/end`)
...
describe("Live broadcasting: chat authorization", () => {
  ...
  it("rejects chatting in a broadcast that has ended", async () => {
    ...
    const host = await registerAndLogin(app, "host");
    const start = await request(app).post("/host/live/broadcasts").set("Authorization", `Bearer ${host.accessToken}`);
    await request(app).post(`/host/live/broadcasts/${start.body.broadcastId}/end`).set("Authorization", `Bearer ${host.accessToken}`);

    const res = await request(app)
      .post(`/host/live/broadcasts/${start.body.broadcastId}/chat`)
      .set("Authorization", `Bearer ${host.accessToken}`)
      .send({ content: "hello?" });
    expect(res.status).toBe(404);
  });
...
describe("Live broadcasting: admin monitoring (admin design follow-up)", () => {
  it("lists currently-live broadcasts with host identity and viewer count, and can force-end one", async () => {
    ...
    const viewer = await registerAndLogin(app, "user");
    const start = await request(app).post("/host/live/broadcasts").set("Authorization", `Bearer ${host.accessToken}`);
    await request(app).post(`/user/live/broadcasts/${start.body.broadcastId}/join`).set("Authorization", `Bearer ${viewer.accessToken}`);
